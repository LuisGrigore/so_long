...
{
	t_image *img_grande = init_image(NULL, NULL, 0, 0, 0, 0, 0);

	img_grande->img = mlx_new_image(screen->mlx, screen->size.x, screen->size.y);
    img_grande->addr = mlx_get_data_addr(img_grande->img, &img_grande->bpp, &img_grande->line_length, &img_grande->endian);
